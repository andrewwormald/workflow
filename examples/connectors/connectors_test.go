...
func TestConnectStreamParallelConsumer(t *testing.T) {
	...
	_, err = workflowB.Await(ctx, foreignID, runID, "Waiting for workflow A")
	jtest.RequireNil(t, err)

	_, err = workflowA.Trigger(ctx, foreignID, "Start")
	jtest.RequireNil(t, err)
	...
	workflow.Require(t, workflowB, foreignID, runID, "Finished waiting", connectors.TypeB{
		Value: "Hello World",
	})
}
